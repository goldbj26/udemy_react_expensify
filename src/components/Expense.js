...
import {RemoveExpenseDB } from '../redux/expenses';
...
const Expense = (props) => (
	...
	<tr id={props.expense.id} className="expense" onClick={() => {props.handle_click(props.expense.id)}}>
		...
		<td className="expense_remove" onClick={ (e) => {e.stopPropagation(); myStore.dispatch(RemoveExpenseDB(props.expense.id));}} >remove</td>
	</tr>		
)
